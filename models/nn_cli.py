"""Interactive CLI for the joint-output NN EV model."""
...
try:
    from .nn_model import MODEL_PATH, TARGET_MODE, FEATURE_VERSION, estimate_all_values, load_model
except ImportError:  # Allows `python models/nn_cli.py`.
    from nn_model import MODEL_PATH, TARGET_MODE, FEATURE_VERSION, estimate_all_values, load_model
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Interactive NN EV predictor")
    ap.add_argument("--model", default=str(MODEL_PATH), help="Path to NN joblib model")
    args = ap.parse_args()
