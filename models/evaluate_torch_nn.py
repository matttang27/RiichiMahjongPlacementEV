"""Evaluate the PyTorch joint-output EV model."""
...
except ImportError:  # Allows `python models/evaluate_torch_nn.py`.
    from evaluate_model import (
        DEFAULT_CALIBRATION_BUCKETS,
        DEFAULT_DB_PATH,
        DEFAULT_ORACLE_MIN_ROUNDS,
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate the PyTorch joint-output EV model")
...
def main(argv: Sequence[str] | None = None) -> dict:
    ...
    print(f"Using PyTorch NN model: {args.model}")

    def load_with_device(path):
        return load_model(path, device=args.device)
